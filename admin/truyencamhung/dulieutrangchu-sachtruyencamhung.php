...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $sql = "SELECT id, title, author, image_path,tomtat,theloai,publisher, release_date, Day, so_luong FROM sachtruyencamhung WHERE id LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%' OR theloai LIKE '%$search%' OR publisher LIKE '%$search%'";
  
} else {
    $sql = "SELECT id, title, author, image_path,tomtat,theloai,publisher, release_date, Day, so_luong FROM sachtruyencamhung";
}
...
<head>
    <meta charset="UTF-8">
    <title>Sách truyền cảm hứng</title>
...
<body>
    ...
    <div class="fixed-menu">
        <h2>Sách truyền cảm hứng</h2>
        <form method="get" action="dulieutrangchu-sachtruyencamhung.php">
        ...
        <a href="add_book-sachtruyencamhung.php"><input type="button" value="Thêm sách mới"></a>
    </div>
    <table class="bang">
    <tr>
        <th>STT</th>
        ...
        while($row = $result->fetch_assoc()) {
          ...
          //nhà xuất bản
            ...
            echo "<td class='action-links'>
                    <a href='edit_book-sachtruyencamhung.php?id=" . $row["id"] . "'>Sửa</a>
                    <a href='delete_book-sachtruyencamhung.php?id=" . $row["id"] . "' onclick='return confirm(\"Bạn có chắc chắn muốn xóa sách này?\")'>Xóa</a>
                  </td>";
            echo "</tr>";
