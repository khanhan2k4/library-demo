...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $sql = "SELECT id, title, author, image_path,tomtat,theloai,publisher, release_date, Day, so_luong FROM truyenkinhdi WHERE id LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%'  OR theloai LIKE '%$search%'  OR publisher LIKE '%$search%'";
} else {
    $sql = "SELECT id, title, author, image_path,tomtat,theloai,publisher, release_date, Day, so_luong FROM truyenkinhdi";
}
...
<div class="fixed-menu">
    <h2>Danh sách sách vừa ra mắt</h2>
        <form method="get" action="dulieutrangchu-truyenkinhdi.php">
        ...
        <a href="add_book-truyenkinhdi.php" ><input type="button" value="Thêm sách mới"></a>
    </div>
    <table class="bang">
    <tr>
        <th>STT</th>
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td class='action-links'>
                    <a href='edit_book-truyenkinhdi.php?id=" . $row["id"] . "'>Sửa</a>
                    <a href='delete_book-truyenkinhdi.php?id=" . $row["id"] . "' onclick='return confirm(\"Bạn có chắc chắn muốn xóa sách này?\")'>Xóa</a>
                  </td>";
            echo "</tr>";
