...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $book_query = "SELECT id, title, author, image_path, tomtat, theloai, publisher, release_date, Day, so_luong FROM sachchuyennganh WHERE id LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%' OR theloai LIKE '%$search%' OR publisher LIKE '%$search%'";
} else {
    $book_query = "SELECT * FROM sachchuyennganh ORDER BY release_date DESC"; // Sắp xếp theo ngày phát hành mới nhất
}
...
<div class="content">
    ...
    <form action="sachchuyennganh.php" method="get">
 ...
 <div id="Sách vừa ra mắt" class="Show_eye">
    ...
    <div class="book-grid">
        <?php while ($book = $book_result->fetch_assoc()): ?>
            <div class="book-item">
                <a href="book-detail-chuyennganh.php?id=<?php echo htmlspecialchars($book['id']); ?>" class="book-link">
