...
// $book_query = "SELECT * FROM sachvanhoaxahoi ORDER BY release_date DESC"; // Sắp xếp theo ngày phát hành mới nhất
...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $book_query = "SELECT id, title, author, image_path, tomtat, theloai, publisher, release_date, Day, so_luong FROM sachvanhoaxahoi WHERE id LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%' OR theloai LIKE '%$search%' OR publisher LIKE '%$search%'";
} else {
    $book_query = "SELECT * FROM sachvanhoaxahoi ORDER BY release_date DESC"; // Sắp xếp theo ngày phát hành mới nhất
}
...
<div class="content">
    ...
    <form action="sachvanhoaxahoi.php" method="get">
 ...
 <div id="Sách vừa ra mắt" class="Show_eye">
    <h3>Sách Văn Hóa- Xã Hội.</h3>
    <div class="book-grid">
        <?php while ($book = $book_result->fetch_assoc()): ?>
            <div class="book-item">
                <a href="book-detail-vanhoaxahoi.php?id=<?php echo htmlspecialchars($book['id']); ?>" class="book-link">
